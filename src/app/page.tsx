...
function CardView({ card, isSelected, onClick, disabled }: { card: Card; isSelected: boolean; onClick: () => void; disabled: boolean }) {
    return (
        <button
          key={card.id}
          onClick={onClick}
          disabled={disabled}
          className={[
            'h-28 w-20 shrink-0 rounded-xl border px-2 py-2 text-left transition-all',
            'bg-white/5',
            !disabled ? 'border-white/20' : 'border-white/5 opacity-50',
            isSelected ? 'outline outline-2 outline-white/80 scale-95' : '',
          ].join(' ')}
          title={card.name}
        >
          <div className="text-[10px] uppercase tracking-tighter opacity-50 font-bold">{card.type}</div>
          <div className="text-xs font-medium leading-tight mt-1 truncate">{card.name}</div>
          <div className="mt-auto flex items-end justify-between">
              <div className="text-[10px] opacity-40">{card.id.split('-').pop()}</div>
              <div className="text-sm font-bold opacity-90">P:{card.power}</div>
          </div>
        </button>
    );
}
          ...
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-[10px] opacity-80"
              >
                Reset
              </button>
            </div>
                                    ...
                                    <div className="flex flex-col items-center animate-in slide-in-from-top-4 duration-500">
                                        ...
                                        <div className="flex gap-2 flex-wrap justify-center">
                                            {state.committedAttackCards.map(c => (
                                                <div key={c.id} className="h-16 w-12 rounded-lg border border-red-500/30 bg-red-500/5 flex flex-col items-center justify-center">
                                                    <span className="text-[8px] opacity-60 uppercase font-black">{c.type[0]}</span>
                                                    <span className="text-xs font-bold">{c.power}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col items-center animate-in slide-in-from-bottom-4 duration-500">
                                        <div className="flex gap-2 flex-wrap justify-center">
                                            {state.committedDefenseCards.map(c => (
                                                <div key={c.id} className="h-16 w-12 rounded-lg border border-blue-500/30 bg-blue-500/5 flex flex-col items-center justify-center">
                                                    <span className="text-[8px] opacity-60 uppercase font-black">{c.type[0]}</span>
                                                    <span className="text-xs font-bold">{c.power}</span>
                                                </div>
                                            ))}
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide no-scrollbar">
                        {state.handYou.map(c => (
                            <CardView 
                                key={c.id} 
                                card={c} 
                                isSelected={state.selectedAttackIds.includes(c.id) || state.selectedDefenseIds.includes(c.id)} 
                                onClick={() => {
